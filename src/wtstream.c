...
wtSTREAM_t* wtOpenFileStream( char* fname, char* mode ) {
  ...
  stream->stream.file.closeable = 1;

  return stream;
}


wtSTREAM_t* wtOpenFileStreamFromHandle( FILE* fp ) {
  wtSTREAM_t* stream;
...
void wtCloseStream( wtSTREAM_t* stream ) {
  switch( stream->type ) {
    case stFILE:
      if( stream->stream.file.closeable ) {
        fclose( stream->stream.file.fp );
      }
      break;
...
void wtPrint( wtSTREAM_t *stream, char *buffer ) {
  ...
  switch( stream->type ) {
    ...
    case stFILE:
      fputs( buffer, stream->stream.file.fp );
      break;
